    ...
    (groups &&
      groups.map((group) => (
        <div key={group.id} style={{ display: "block", width: 200 }}>
          <Link href={`${pathname}?region=${group.id}`} passHref>
            <MenuLink
              bg="whiteBg"
              color="black"
...
const makeStatsDashboard = ({ queryKey, fileUrl }) => () => {
  const router = useRouter();
  const { region = "national" } = router.query;
  const regionId = region.replace(/^\//, "");
  const { data } = useQuery(queryKey, () => fetch(fileUrl).then((r) => r.json()));
  return (
    <Box bg="white" p={4}>
      <Head>
...
export default makeStatsDashboard;
